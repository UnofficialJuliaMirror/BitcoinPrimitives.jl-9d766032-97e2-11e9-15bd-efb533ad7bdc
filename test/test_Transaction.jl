...
@testset "Transaction" begin
    ...
    @testset "Serialize" begin
        ...
        tx = Tx(IOBuffer(raw_bip141))
        println(bytes2hex(serialize(tx)))
    ...
    #     index = 0
    #     want = 42505594
    #     tx_in = TxIn(hex2bytes(tx_hash), index)
    #     @test value(tx_in) == want
    # end
    ...
    #     tx_in = TxIn(hex2bytes(tx_hash), index)
    #     @test serialize(scriptpubkey(tx_in)) == want
    # end
    # @testset "Fee" begin
    #     raw_tx = hex2bytes("")
    ...
    #     push!(tx_outs, TxOut(Int(0.99 * 100000000), p2pkh_script(h160)))
    #     h160 = base58checkdecode(b"mnrVtF8DWjMu839VW3rBfgYaAfKk8983Xf")[2:end]
    #     push!(tx_outs, TxOut(Int(0.1 * 100000000), p2pkh_script(h160)))
